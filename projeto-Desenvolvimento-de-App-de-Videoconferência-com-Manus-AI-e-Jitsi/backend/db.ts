...
import { InsertUser, users, tasks, InsertTask, Task, courses, InsertCourse, Course, works, InsertWork, Work, userConnections, InsertUserConnection, UserConnection, userProfiles, InsertUserProfile, UserProfile, messages, InsertMessage, Message, scheduledConferences, InsertScheduledConference, ScheduledConference } from "../drizzle/schema";
...
export async function getUserMessages(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(messages).where(eq(messages.recipientId, userId));
}
...
export async function scheduleConference(data: InsertScheduledConference) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(scheduledConferences).values(data);
  return result[0].insertId;
}

export async function getScheduledConferences(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(scheduledConferences).where(eq(scheduledConferences.userId, userId));
}

export async function getConferenceById(id: number) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(scheduledConferences).where(eq(scheduledConferences.id, id)).limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function updateConferenceStatus(id: number, status: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(scheduledConferences).set({ status: status as any }).where(eq(scheduledConferences.id, id));
}

export async function markReminderSent(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(scheduledConferences).set({ reminderSent: true }).where(eq(scheduledConferences.id, id));
}

export async function deleteConference(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(scheduledConferences).where(eq(scheduledConferences.id, id));
}
